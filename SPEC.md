# Claritix - Website Specification v0.2

**Status:** Current production baseline with a new UX enhancement brief in `docs/CLARITIX_UX_SPEC.md`.
**Purpose:** Single source of truth for building the Claritix marketing site. Every section is either **LOCKED** (decided in planning), **PROPOSED** (recommendation, review before build), or **OPEN** (decision required before or during build).

**Brand update (2026-06):** The site launched as **Claritix** at `claritix.co`. Older references to InsightOut in milestone history are superseded by this current brand state.
...
| Element | Value |
|---|---|
| Company name | **Claritix** |
| Brand spine (hero triad) | **Product • Analytics • Strategy** |
| Sub-positioning line | *Your product and analytics function, on demand.* |
| Core concept | Product, behavioral, business, and market evidence converge into clarity leaders can act on. The brand promise is analytical depth carried into decisions. |
...
| Tone of voice | Senior, direct, analytical. No agency fluff, no buzzword inflation. Confident statements over adjectives. **Company voice (Claritix / "we") everywhere - no first person singular anywhere, including About/Approach** (reversed at M3b, 2026-06; supersedes the earlier first-person allowance). |

**Domain:** `claritix.co` is live on GitHub Pages.
...
| # | Section | Content |
|---|---|---|
| 1 | **Hero** | Claritix wordmark/logo · triad **Product • Analytics • Strategy** as the visual lockup · sub-line *"Your product and analytics function, on demand."* · single primary CTA ("Let's talk") |
...
| 6 | **Closing CTA** | "Work with Claritix" - name, email, one-line message. |
...
### Palette - LOCKED
Final tokens are derived from the Claritix brand system (see §6a):
...
## 6a. Logo Integration - CURRENT

The active site uses the Claritix wordmark and logo assets in `public/brand/claritix/`, with generated favicon, app icon, and OG image assets in `public/`.

**Production assets:**
| File | Use |
|---|---|
| `public/brand/claritix/claritix-wordmark.webp` | Wordmark on nav, hero, footer |
| `public/brand/claritix/claritix-logo.png` | Source/reference logo asset |
| `public/favicon.*`, `public/icon-*`, `public/og-image.png` | Generated production metadata assets |

Future brand-asset changes should be explicit milestone work because the current site depends on these dimensions and treatments.
...
- Realistic expectation: Claritix search visibility will be built through network, LinkedIn, and targeted brand/search compounds, not cold generic SEO alone. Optimize for *Claritix + product analytics/advisory* compounds and for converting visitors who arrive with intent.
...
| Domain + trademark check result | Everything | ✓ domain live at `claritix.co` |
...
**Blocking before M5:**
- [x] Name/domain check (§1) - `claritix.co` live
